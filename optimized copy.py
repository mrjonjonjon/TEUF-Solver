...
def run():
    global equalities,disequalities,graph,predecessors,eq_class_to_pred_list,sig_table,uf,vertex_counter,term_to_vert
    xpoints=np.linspace(0,10000,100)
    ypoints=[]
    for v in range(0,1):#xpoints:
        ...
        uf=0
        vertex_counter=0
        _,_,equalities,disequalities = 1,2,vvv,[]#generate_test(num_vars=1000,num_funcs=10,num_equalities = int(x),num_inequalities=0)
        ...
        for (a,b) in disequalities:
            if a in term_to_vert and b in term_to_vert and uf.connected(term_to_vert[a],term_to_vert[b]):
                print("UNSAT")
                t1 = time.time()
        print("RESULT IS: SAT\nEQUIVALENCE CLASSES ARE: ",uf.components())
        t1=time.time()
        ypoints.append(t1-t0)
    #plt.plot(xpoints,ypoints,label='optimized')
    #plt.show()
    #print("zzz: ",uf.components())
    #or q in uf.components():
        #print(q,'\n')

run()
#naiverun()
#plt.title("Comparing Naive and Optimized implementations of a TEUF solver ")
